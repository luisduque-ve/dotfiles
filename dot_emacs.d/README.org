...
- [[#find-file-with-fd][Find file with fd]]
...
* [[https://github.com/manateelazycat/lsp-bridge?tab=readme-ov-file#installation][LSP Bridge]]

LSP Bridge must be cloned first, I decided to have it in
~/.emacs.d/lsp-bridge

#+begin_src shell :tangle no
  git clone https://github.com/manateelazycat/lsp-bridge ~/.emacs.d/lsp-bridge
#+end_src

For the python dependencies setup just run the shell function pysetup.
...
#+begin_src elisp
  ...
  (use-package lsp-bridge
    :ensure nil ;; Prevents package manager installation
...
* Find file with fd
This is a utility function to find file using fd very similar as
is done in terminal based workflows.
#+begin_src elisp
  (defvar find-files-program
    (if (executable-find "fd")
	'("fd" "--type" "f" "--hidden")
      (error "`fd` is not available on your system"))
    "Command to find files recursively using `fd`.")

  (defun find-file-rec ()
    "Find a file in the current working directory recursively using `fd`."
    (interactive)
    (find-file
     (completing-read "Find file: "
		      (apply #'process-lines find-files-program))))

  ;; Key mapping for `find-file-rec`
  (global-set-key (kbd "C-c f") 'find-file-rec)
#+end_src
